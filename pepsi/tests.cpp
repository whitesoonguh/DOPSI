...
std::vector<int64_t> genDataPEPSI(
    int32_t numItem
) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int64_t> dist(3, ((int64_t)(1) << 24) - 1);
...
void testPEPSIProtocol(
  ...
  bool isEncrypted
    ...
    std::vector<int64_t> msgVec = genDataPEPSI(1<<numItem);
...
void testPEPSIProtocolPSI(
  ...
  bool isEncrypted
    ...
    std::cout << "Step 1-3: Server Side Preprocessing" << std::endl;
    PEPSIDB serverDB = constructPEPSIDBPSI(
      bfv, msgVec, bitlen, HW, isEncrypted
    );
    ...
    std::cout << "Step 2: Client Side Computation" << std::endl;
    std::cout << "Step 3: Query Encryption" << std::endl;
    std::vector<int64_t> queryVec = genDataPEPSI(2048);
    PEPSIQuery query = encryptClientDataPSI(bfv, queryVec, bitlen, HW);
    size_t querySize = ctxtSize(query.payload[0]) * query.numCtxt;
